...
class DragItemRecyclerView : RecyclerView, AutoScrollListener {
     ...
     */
    private fun findChildView(x: Float?, y: Float?): View? {
    ...
    private fun updateDragPositionAndScroll() {
        val view = findChildView(mDragItem?.x, mDragItem?.y)
        view?.let {
            ...
            val linearLayoutManager =
                layoutManager is LinearLayoutManager && layoutManager !is GridLayoutManager
            if (linearLayoutManager) {
                ...
                val movedPassedCenterY =
                    if (dragDown) mDragItem?.y!! > viewCenterY else mDragItem?.y!! < viewCenterY

                // If new height is bigger then current and not passed centerY then reset back to current position
                if (viewHeight > mDragItem?.dragItemView?.measuredHeight!! && !movedPassedCenterY) {
            ...
            if (layoutManager?.orientation == LinearLayoutManager.VERTICAL) {
                if (mDragItem?.y!! > height - view.height / 2 && !lastItemReached) {
                    mAutoScroller!!.startAutoScroll(ScrollDirection.UP)
                } else if (mDragItem?.y!! < view.height / 2 && !firstItemReached) {
                    mAutoScroller!!.startAutoScroll(ScrollDirection.DOWN)
                } else {
                    mAutoScroller!!.stopAutoScroll()
                }
            } else {
                if (mDragItem?.x!! > width - view.width / 2 && !lastItemReached) {
                    mAutoScroller!!.startAutoScroll(ScrollDirection.LEFT)
                } else if (mDragItem?.x!! < view.width / 2 && !firstItemReached) {
    ...
    fun startDrag(itemView: View, itemId: Long, x: Float, y: Float): Boolean {
        ...
        if (!isDragEnabled || mCanNotDragAboveTop && dragItemPosition == 0 || mCanNotDragBelowBottom && dragItemPosition == mAdapter!!.itemCount - 1) {
            return false
        }
        ...
        mAdapter?.notifyDataSetChanged()
        if (mListener != null) {
            mListener!!.onDragStarted(mDragItemPosition, mDragItem?.x!!, mDragItem?.y!!)
        }
        invalidate()
        return true
    }
    ...
    fun onDragEnded() {
        ...
        isEnabled = false
        if (mDisableReorderWhenDragging) {
            val newPos = mAdapter!!.getPositionForItemId(mAdapter?.dropTargetId!!)
